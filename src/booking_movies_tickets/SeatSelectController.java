package booking_movies_tickets;

import javafx.application.Platform;
import javafx.event.ActionEvent;
import javafx.fxml.FXML;
import javafx.scene.Scene;
import javafx.scene.control.*;
import javafx.scene.layout.GridPane;
import javafx.stage.Stage;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.Collections;
import java.util.Comparator;

public class SeatSelectController {
    private Round round;

    ArrayList<Button> buttonsList = new ArrayList<>();
    ArrayList<String> chair = new ArrayList<>();
    Button button[][] = new Button[15][20];
    CinemaOperator cinema = CinemaOperator.getInstance();

    @FXML GridPane chairGridPane;

    @FXML public void initialize() {
        Platform.runLater(new Runnable() {
            @Override
            public void run() {

                char alphabet = 65+14;
                for (int i = 0; i < 20; i++) {
                    for (int j = 0; j < 15; j++) {
                        button[j][i] = new Button();
                        button[j][i].setMaxSize(44, 30);
                        button[j][i].setStyle("-fx-background-image: url('/image/chair-1-2-3-4-22.png');-fx-background-size: 44 30;-fx-background-position: center center");
                        if(i<10){button[j][i].setId(""+(char)(alphabet-j)+"0"+i);}
                        else{button[j][i].setId((""+(char)(alphabet-j)+i));}
                        button[j][i].setOnAction(event -> handleChairBtn(event));
                        chairGridPane.add(button[j][i], i, j, 1, 1);
                    }
                }
            }
        });
    }

    public void setRound(Round round) {
        this.round = round;
    }


    @FXML public void handleChairBtn(ActionEvent e) {
        Button b = (Button) e.getSource();
        System.out.println(b.getId());
        //System.out.println(b);
        if(buttonsList.contains(b)){
            buttonsList.remove(b);
            b.setStyle("-fx-background-image: url('/image/chair-1-2-3-4-22.png');-fx-background-size: 39 25;-fx-background-position: center center");
            chair.remove(b.getId());
        }
        else{
            buttonsList.add(b);
            b.setStyle(null);
            chair.add(b.getId());
        }
        Collections.sort(chair);
        System.out.println(chair);
    }















}
